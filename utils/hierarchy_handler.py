...
class HierarchyFactory:
    ...
    def ls(self, path: Optional[str] = None) -> List[str]:
        ...
        if target_path in self.main_graph:
            ...
            sections = [handler.get_name() for handler in self.main_graph.nodes[target_path]['data']]
            # List subdirectories
            subdirs = [node.rsplit('/', 1)[-1] for node in self.main_graph.successors(target_path)]
