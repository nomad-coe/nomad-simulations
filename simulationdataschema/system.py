    ...
    def normalize(self, archive: EntryArchive, logger) -> None:
        ...
        if atom_labels is None:
            if atomic_numbers is None:
                logger.error('System.atomic_cell has neither labels nor atomic_numbers defined.')
                return
            try:
                atom_labels = [ase.data.chemical_symbols[number] for number in atomic_numbers]
            except IndexError:
                logger.error('System.atomic_cell has atomic_numbers that are out of range of the periodic table.')
                return
        ...
        if atom_labels != list(chemical_symbols):
            logger.warning('Chemical symbols in System.atomic_cell.labels are ambigous and cannot be '
                           'recognized by ASE.')
        if atomic_numbers is None:
            atomic_numbers = ase_atoms.get_atomic_numbers()
        else:
            if atomic_numbers != list(ase_atoms.get_atomic_numbers()):
                logger.info('The parsed System.atomic_cell.atomic_numbers do not coincide with '
        ...
        if pbc is None:
            ...
            logger.info('Could not find System.atomic_cell.periodic_boundary_conditions information. '
        ...
        if len(atom_positions) != len(atom_labels):
            logger.error('Length of System.atomic_cell.positions does not coincide with the length '
        ...
        if lattice_vectors is None:
            ...
            if lattice_vectors_reciprocal is None:
                self.lattice_vectors_reciprocal = 2 * np.pi * ase_atoms.get_reciprocal_cell() / ureg.angstrom
...
class ModelSystem(System):
    ...
    type = Quantity(
        type=MEnum('atom', 'molecule / cluster', 'bulk', 'surface', '2D', '1D', 'active_atom', 'unavailable'),
        description='''
    ...
    def _resolve_system_type_and_dimensionality(self, ase_atoms: ase.Atoms) -> str:
        ...
        if len(ase_atoms) <= config.normalize.system_classification_with_clusters_threshold:
            try:
                ...
                self.logger.warning('MatID system classification failed.', exc_info=e, error=str(e))
            ...
            self.logger.info("ModelSystem.type and dimensionality analysis not run due to large system size.")
    ...
    def _resolve_bulk_symmetry(self, ase_atoms: ase.Atoms) -> None:
        ...
        symmetry = {}
        try:
            symmetry_analyzer = SymmetryAnalyzer(ase_atoms, symmetry_tol=config.normalize.symmetry_tolerance)
            ...
            symmetry['space_group_symbol'] = symmetry_analyzer.get_space_group_international_short()
            ...
            standard_equivalent_atoms = symmetry_analyzer.get_equivalent_atoms_conventional()
            ...
            self.logger.debug('Symmetry analysis with MatID is not available.', details=str(e))
            return
        except Exception as e:
        ...
        if symmetry.get('space_group_number'):
            ...
            aflow_prototype = search_aflow_prototype(symmetry.get('space_group_number'), norm_wyckoff)
    ...
    def normalize(self, archive: EntryArchive, logger) -> None:
        ...
        if len(self.atomic_cell) == 0:
            self.logger.warning('Could not find the originally parsed atomic system. '
        ...
        if original_atom_positions is not None:
            ...
            self.dimensionality = 'unavailable' if not self.dimensionality else self.dimensionality
            self.type, self.dimensionality = self._resolve_system_type_and_dimensionality(ase_atoms)
            ...
            for atom_cell in self.atomic_cell:
                ...
                atom_cell.alpha, atom_cell.beta, atom_cell.gamma = cell.angles() * ureg.degree
        ...
        try:
            ...
            self.logger.warning('Could not extract the chemical formulas information.', exc_info=e, error=str(e))
